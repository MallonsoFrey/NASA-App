...
async function marsRender(num, cam) {
  if (cam === "" || num > 4300 || num <= 0) {
    output.innerHTML = `
        <div class="mars-container">
            <p class="mars__error-message">Please enter valid Sol or choose a camera before proceeding.</p> 
  ...
  try {
    const response = await fetch(
      `https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol=${num}&camera=${cam}&api_key=ujhHC5aAHogxEryayLSB4H3LRoKlpAkpqq48JZ8g`
    );
    ...
    if (!data || !data.photos || data.photos.length === 0) {
      output.innerHTML = `
                <div class="mars-container">
                    <p class="mars__error-message">No photos available for this Sol or camera. Please try another selection.</p>
                    <button class="returnButton buttonStyle">Go back</button>
                </div>
            `;
      const goBackButton = document.querySelector(".returnButton");
    ...
    output.innerHTML = `
            <div class="mars-container">
                ...
                <div class="mars-container__photo"><img style="max-width: 100%; height: auto;" src="${data.photos[0].img_src}" alt="Mars Rover Photo"></div>
    ...
    output.innerHTML = `
        <div class="mars-container">
            <p class="mars__error-message">Error fetching data: ${error.message}</p>
            <button class="returnButton buttonStyle">Go back</button>
        </div>
        `;
    const goBackButton = document.querySelector(".returnButton");
