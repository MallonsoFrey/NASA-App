export function initializeEpicPage() {
  ...
  //  Проверка на дублирование загрузки стилей 
  ...
  function loadGallery() {
    ...
    // Убираем чёрный фон до загрузки
    photoContainer.classList.remove('black-background');
    galleryContainer.classList.remove('black-background');
    ...
    fetch(`https://epic.gsfc.nasa.gov/api/natural/date/${dateInput}?api_key=${apiKey}`)
      ...
      .then(data => {
        ...
        // Добавляем чёрный фон после успешной загрузки изображений
        photoContainer.classList.add('black-background');
        galleryContainer.classList.add('black-background');
        ...
        // Добавляем кнопку "Close Gallery" после загрузки галереи
  ...
  // Функция для добавления кнопки "Close Gallery"
  function addCloseGalleryButton() {
    if (!document.getElementById('closeGalleryBtn')) {
      ...
      closeGalleryBtn.textContent = '×'; // Добавляем крестик

      // Добавляем кнопку в контейнер с фото
      const photoContainer = document.getElementById('photo-container');
      photoContainer.appendChild(closeGalleryBtn);

      // Добавляем обработчик клика для закрытия галереи
      closeGalleryBtn.addEventListener('click', closeGallery);
    }
  }

  // Функция для закрытия галереи и возврата на главную страницу
  function closeGallery() {
    mainContent.innerHTML = routes.spaceGaze.content; // Вернуть основной контент (главную страницу)
  }
});
}